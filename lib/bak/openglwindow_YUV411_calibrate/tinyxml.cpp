...
//#include "modules/surroundview/openglwindow/tinyxml.h"
#include "tinyxml.h"
...
void TiXmlBase::EncodeString(const TIXML_STRING &str, TIXML_STRING *outString) {
  ...
  while (i < static_cast<int>(str.length())) {
    ...
    if (c == '&' && i < (static_cast<int>(str.length()) - 2) &&
        str[i + 1] == '#' && str[i + 2] == 'x') {
...
#else
      snprintf(buf, "&#x%02X;", (unsigned)(c & 0xff));
#endif
