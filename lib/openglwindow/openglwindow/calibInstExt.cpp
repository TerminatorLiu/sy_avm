...
//*****************************************************
extern int extCalibError;
//extern sem_t g_sem_namualCalib_done;

CvPoint2D32f manualCorners[4];
unsigned char* manualCalibImg;
volatile int manualCalibFlag =0;
...
int run_calib_InstExt(const CvMat* img_src,const CvSeq* image_points_seq,CvMat* camera_matrix, 
				  CvMat* dist_coeffs, CvMat* extr_params,int cam_id, PARA_FIELD para_field)
{
	...
	CvSize board_size = cvSize(para_field.chessboard_length_corners -1, para_field.chessboard_width_corners -1);
	...
	//IplImage* rectify0 = cvCreateImage(cvSize(RECTIFY_HEIGHT,RECTIFY_HEIGHT), IPL_DEPTH_8U, 3);
	//IplImage* rectify1 = cvCreateImage(cvSize(RECTIFY_HEIGHT,RECTIFY_HEIGHT), IPL_DEPTH_8U, 3);
	

	fx = 317.6896908038024 / 2;
	fy = 414.1124332021072 / 2;

	_dist_coeffs[0]   =  0.07180911227878471;
	_dist_coeffs[1]   =  0.046776483620016136;
	_dist_coeffs[2]   =  -0.030762048740503825;
	_dist_coeffs[3]   =  0.0047799053814934584;

#if 0
	fx = 312.246273/2;
	fy = 314.530830/2;
	_dist_coeffs[0]   =  0.071289;
	_dist_coeffs[1]   =  0.030682;
	_dist_coeffs[2]   =  -0.007719;
	_dist_coeffs[3]   =  -0.003017;
#endif
	...
	initChessBoardPoints(cam_id,image_points_seq,image_points0,object_points0,para_field);
#if 1
	...
	//*************project square0***************
	initObjectPoints(obj_FullPoints,output_full_size,cam_id,para_field,real2pix_ratio,0);
	...
	found0 = cvFindSquareCorners( img_rectify0, square0, para_field, real2pix_ratio,cam_id ,0);		
	//*************project square1***************
	initObjectPoints(obj_FullPoints,output_full_size,cam_id,para_field,real2pix_ratio,1);
	cvProjectPoints_fisheyeD1(obj_FullPoints,rot_vects,trans_vects,&camera,&dist_coeffs0,project_Points,NULL,NULL,NULL,NULL,NULL);
	interpolate_image(img_src,output_full_size,img_rectify1,project_Points);
	// find the square1 corners		
	found1 = cvFindSquareCorners( img_rectify1, square1, para_field, real2pix_ratio,cam_id ,0);
	...
	//manualCalibFlag = 1;
#if 0
	cvCvtColor(img_rectify0, rectify0, CV_GRAY2BGR);
	cvCvtColor(img_rectify1, rectify1, CV_GRAY2BGR);

	cvCircle(rectify0, cvPoint(square0[0].x, square0[0].y), 3, CV_RGB(255, 0, 0), -1, 8);  //?$)A;e?
	cvCircle(rectify0, cvPoint(square0[1].x, square0[1].y), 3, CV_RGB(0, 255, 0), -1, 8);  //?$)A;e?
	cvCircle(rectify0, cvPoint(square0[2].x, square0[2].y), 3, CV_RGB(0, 0, 255), -1, 8);  //?$)A;e?
	cvCircle(rectify0, cvPoint(square0[3].x, square0[3].y), 3, CV_RGB(255, 255, 0), -1, 8);  //?$)A;e?

	cvCircle(rectify1, cvPoint(square1[0].x, square1[0].y), 3, CV_RGB(255, 0, 0), -1, 8);  //?$)A;e?
	cvCircle(rectify1, cvPoint(square1[1].x, square1[1].y), 3, CV_RGB(0, 255, 0), -1, 80);  //?$)A;e?
	cvCircle(rectify1, cvPoint(square1[2].x, square1[2].y), 3, CV_RGB(0, 0, 255), -1, 8);  //?$)A;e?
	cvCircle(rectify1, cvPoint(square1[3].x, square1[3].y), 3, CV_RGB(255, 255, 0), -1, 8);  //?$)A;e?

	cvNamedWindow("rectify0",1);
    cvShowImage("rectify0",rectify0);
	cvNamedWindow("rectify1",1);
    cvShowImage("rectify1",rectify1);

    cvWaitKey(0);
#endif

manual_calib:
	if( manualCalibFlag) //if do not find two squares , find two squares manually
	{
		int step = 20;
		int j;
		float avg_x,avg_y;
		manualCalibImg = img_rectify0->data.ptr;
		if(found0)
		{
			for(i = 0; i<4; i++)
			{
				manualCorners[i].x = square0[i].x;
				manualCorners[i].y = square0[i].y;
			}	
		}
		else
		{
			manualCorners[0].x = RECTIFY_WIDTH/2 -step;
			manualCorners[0].y = RECTIFY_HEIGHT/2 -step;
			manualCorners[1].x = RECTIFY_WIDTH/2 +step;
			manualCorners[1].y = RECTIFY_HEIGHT/2 -step;
			manualCorners[2].x = RECTIFY_WIDTH/2 +step;
			manualCorners[2].y = RECTIFY_HEIGHT/2 +step;
			manualCorners[3].x = RECTIFY_WIDTH/2 -step;
			manualCorners[3].y = RECTIFY_HEIGHT/2 +step;
		}
		//sem_wait(&g_sem_namualCalib_done);
		printf("corner0 manual set !\n");
		/* sort corners0*/
		// rearrange the square corners
		avg_x = (manualCorners[0].x + manualCorners[1].x + 
					manualCorners[2].x + manualCorners[3].x)*0.25 ;
		avg_y = (manualCorners[0].y + manualCorners[1].y + 
					manualCorners[2].y + manualCorners[3].y)*0.25 ;
		for(j =0; j <4; j++)
		{
			float temp_x = manualCorners[j].x ,
				temp_y = manualCorners[j].y ;
		...
		found0 = 1;
		manualCalibImg = img_rectify1->data.ptr;
		if(found1)
		{
			for(i = 0; i<4; i++)
			{
				manualCorners[i].x = square1[i].x;
				manualCorners[i].y = square1[i].y;
			}	
		}
		else
		{
	...
	if(1)
	{
		...
		mergePoints(square0,square1,real2pix_ratio,cam_id,para_field,rot_vects,
					trans_vects,&camera,&dist_coeffs0,img_pt_dst,obj_pt_dst);
#if 0
		CvPoint2D32f* img_pt = (CvPoint2D32f*) image_points->data.fl;
		CvMat* img_debug = cvCreateMat(img_src->rows,img_src->cols,CV_8UC1);
		cvCopy(img_src,img_debug,NULL);
		for(i = 0; i < point_count + 4*square_count  ; i++)
		{
			if(img_pt[i].y > 4 && img_pt[i].y < img_debug->rows-4 && img_pt[i].x >4 && img_pt[i].x < img_debug->cols-4)
			{			
				cvSet2D(img_debug,img_pt[i].y*2,img_pt[i].x*2+1,cvScalar(255));
				cvSet2D(img_debug,img_pt[i].y*2,img_pt[i].x*2-1,cvScalar(255));
				cvSet2D(img_debug,img_pt[i].y*2+1,img_pt[i].x*2,cvScalar(255));
				cvSet2D(img_debug,img_pt[i].y*2-1,img_pt[i].x*2,cvScalar(255)); 
			}
		}
		FILE* fid = fopen("/root/img_debug.txt","wb+");
				fwrite(img_debug->data.ptr,WIDVPFE*HEIVPFE,1,fid);
				fclose(fid);
		cvReleaseMat( &img_debug);
#endif
		...
		#if 0
		if(!manualCalibFlag &&  reprojection_err >=12 && !checkTempFlag)
		{
			manualCalibFlag = 1;
			printf("instParaOpt error\n");
			goto manual_calib;
		}
		#endif
	...
	printf("reprojection_err 111 = %f\n",reprojection_err);
	return ret;

}

void initObjectPoints(CvMat* obj_FullPoints, CvSize output_size,int cam_id,
				 PARA_FIELD para_field,float real2pix_ratio,int square_idx)
{
	...
	case 2:
	{
		if(square_idx == 0)
		{
			offset_x = out_width*0.5 * real2pix_ratio + (HALF_BIG_SQUARE+CHESS_BOARD_TO_LEFT_BIG_SQUARE_DISTANCE+para_field.chessboard_length_corners * para_field.square_size * 0.5)-(para_field.chessboard_width_corners * para_field.square_size+para_field.car_width/2); // move the square to the center of the image;
			offset_y = HALF_BIG_SQUARE + CAR_F_BLIND + para_field.LRchess2carFront_distance + out_height*0.5 * real2pix_ratio;
		}
		else if(square_idx == 1)
		{
			offset_x = (HALF_BIG_SQUARE+CHESS_BOARD_TO_LEFT_BIG_SQUARE_DISTANCE+para_field.chessboard_length_corners * para_field.square_size * 0.5)-(para_field.chessboard_width_corners * para_field.square_size+para_field.car_width/2) + out_width*0.5 * real2pix_ratio; // move the square1 to the center of the image;
			offset_y = -(para_field.car_length - para_field.LRchess2carFront_distance + CAR_B_BLIND + HALF_BIG_SQUARE)+ out_height*0.5 * real2pix_ratio ;
		}
		break;
	}
	case 3:
	{
	 	/*if(square_idx == 0)
        {
            ...
            offset_x = out_width * 0.5 * real2pix_ratio + para_field.car_width * 0.5  - (70 + 40) -50;				// move the square to the center of the image;
            offset_y = 50 + para_field.LRchess2carFront_distance + out_height * 0.5 * real2pix_ratio;
        ...
        else if(square_idx == 1)
        {
            ...
            offset_x = out_width * 0.5 * real2pix_ratio + para_field.car_width * 0.5 - (70 + 40) -50; 				// move the square to the center of the image;
            offset_y = -(para_field.car_length - para_field.LRchess2carFront_distance + 50) + out_height * 0.5 * real2pix_ratio ;

        }*/

        if(square_idx == 0)
        {
                offset_x = out_width * 0.5 * real2pix_ratio + para_field.car_width * 0.5  - (HALF_BIG_SQUARE + CHESS_BOARD_TO_LEFT_BIG_SQUARE_DISTANCE + para_field.chessboard_length_corners * para_field.square_size * 0.5); 				// move the square to the center of the image;
                offset_y = (HALF_BIG_SQUARE + CAR_F_BLIND) + para_field.LRchess2carFront_distance + out_height * 0.5 * real2pix_ratio;
        }
        else if(square_idx == 1)
        {
                offset_x = out_width * 0.5 * real2pix_ratio + para_field.car_width * 0.5 - (HALF_BIG_SQUARE + CHESS_BOARD_TO_LEFT_BIG_SQUARE_DISTANCE + para_field.chessboard_length_corners * para_field.square_size * 0.5); 				// move the square to the center of the image;
                offset_y = -(HALF_BIG_SQUARE + para_field.car_length - para_field.LRchess2carFront_distance + CAR_B_BLIND) + out_height * 0.5 * real2pix_ratio ;
        }
		break;
	}
	default:
...
interpolate_image(const CvMat* img_src,CvSize output_size, CvMat* img_dst, CvMat* project_Points )
{
	...
	unsigned char* img_src_pt = img_src->data.ptr;
	unsigned char* img_dst_pt = img_dst->data.ptr;
...
int mergePoints(CvPoint2D32f *square0,CvPoint2D32f *square1, float real2pix_ratio,
				int cam_id,PARA_FIELD para_field,CvMat* rot_vects,CvMat* trans_vects,
				...
				CvPoint2D32f *img_pt_dst0,CvPoint3D32f *obj_pt_dst0)
{
	...
	int square_num =0;
	int point_count = (para_field.chessboard_length_corners -1) * (para_field.chessboard_width_corners -1);
	...
	case 2:
	    {
	        offset_x0 = out_width * 0.5 * real2pix_ratio  - para_field.car_width * 0.5 - 				// move the square to the center of the image;
	                        para_field.chessboard_width_corners * para_field.square_size + (HALF_BIG_SQUARE + CHESS_BOARD_TO_LEFT_BIG_SQUARE_DISTANCE + para_field.chessboard_length_corners * para_field.square_size * 0.5);
	        offset_y0 = (HALF_BIG_SQUARE + CAR_F_BLIND) + para_field.LRchess2carFront_distance + out_height * 0.5 * real2pix_ratio;
	        offset_x1 = out_width * 0.5 * real2pix_ratio  - para_field.car_width * 0.5 - 				// move the square to the center of the image;
	                    para_field.chessboard_width_corners * para_field.square_size + (HALF_BIG_SQUARE + CHESS_BOARD_TO_LEFT_BIG_SQUARE_DISTANCE + para_field.chessboard_length_corners * para_field.square_size * 0.5);
	        offset_y1 = -(para_field.car_length - para_field.LRchess2carFront_distance + CAR_B_BLIND + HALF_BIG_SQUARE) + out_height * 0.5 * real2pix_ratio ;

	        square_base_x  = para_field.chessboard_width_corners * para_field.square_size + para_field.car_width * 0.5 - (para_field.chessboard_length_corners * para_field.square_size * 0.5 + CHESS_BOARD_TO_LEFT_BIG_SQUARE_DISTANCE + BIG_SQUARE);
	        square_base_y  =  -(para_field.LRchess2carFront_distance + CAR_F_BLIND);
	        square1_base_x = para_field.chessboard_width_corners * para_field.square_size + para_field.car_width * 0.5 - (para_field.chessboard_length_corners * para_field.square_size * 0.5 + CHESS_BOARD_TO_LEFT_BIG_SQUARE_DISTANCE + BIG_SQUARE);
	        square1_base_y = (para_field.car_length - para_field.LRchess2carFront_distance + CAR_B_BLIND + BIG_SQUARE );
	    	break;
	    }
	case 3:
    {
        offset_x0 = out_width * 0.5 * real2pix_ratio + para_field.car_width * 0.5 - (HALF_BIG_SQUARE + CHESS_BOARD_TO_LEFT_BIG_SQUARE_DISTANCE + para_field.chessboard_length_corners * para_field.square_size * 0.5); 				// move the square to the center of the image;
        offset_y0 = (HALF_BIG_SQUARE + CAR_F_BLIND) + para_field.LRchess2carFront_distance + out_height * 0.5 * real2pix_ratio;
        offset_x1 = out_width * 0.5 * real2pix_ratio + para_field.car_width * 0.5 - (HALF_BIG_SQUARE + CHESS_BOARD_TO_LEFT_BIG_SQUARE_DISTANCE + para_field.chessboard_length_corners * para_field.square_size * 0.5); 				// move the square to the center of the image;
        offset_y1 = -(para_field.car_length - para_field.LRchess2carFront_distance + CAR_B_BLIND + HALF_BIG_SQUARE) + out_height * 0.5 * real2pix_ratio ;

        square_base_x  = -para_field.car_width * 0.5 + (para_field.chessboard_length_corners * para_field.square_size * 0.5 + CHESS_BOARD_TO_LEFT_BIG_SQUARE_DISTANCE);
        square_base_y  = -(para_field.LRchess2carFront_distance + CAR_F_BLIND);
        square1_base_x = -para_field.car_width * 0.5 + (para_field.chessboard_length_corners * para_field.square_size * 0.5 + CHESS_BOARD_TO_LEFT_BIG_SQUARE_DISTANCE);
        square1_base_y = (para_field.car_length - para_field.LRchess2carFront_distance + CAR_B_BLIND + BIG_SQUARE );
