...
class TiXmlBase {
 ...
 protected:
  ...
  inline static bool IsWhiteSpace(int c) {
    if (c < 256) return IsWhiteSpace(static_cast<char>(c));
...
class TiXmlAttribute : public TiXmlBase {
  ...
  */
  ...
  /*	Attribute parsing starts: first letter of the name
                                       returns: the next char after the value
     end quote
  */
...
class TiXmlComment : public TiXmlNode {
 public:
  ...
  explicit TiXmlComment(const char *_value)
...
#ifdef TIXML_USE_STL
  /// Constructor.
  explicit TiXmlText(const std::string &initValue)
      : TiXmlNode(TiXmlNode::TINYXML_TEXT) {
    SetValue(initValue);
    cdata = false;
  }
