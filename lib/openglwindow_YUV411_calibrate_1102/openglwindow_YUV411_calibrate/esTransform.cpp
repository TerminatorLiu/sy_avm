 ...
 *****************************************************************************/
//#include "modules/surroundview/openglwindow/esTransform.h"
...
修改记录：
====================================================================*/
void esRotate(ESMatrix *result, GLfloat angle, GLfloat x, GLfloat y,
              GLfloat z) {
...
void esMatrixLookAt(ESMatrix *result, float posX, float posY, float posZ,
                    float lookAtX, float lookAtY, float lookAtZ, float upX,
                    float upY, float upZ) {
  ...
  // normalize axisZ
  length =
      sqrtf(axisZ[0] * axisZ[0] + axisZ[1] * axisZ[1] + axisZ[2] * axisZ[2]);
  ...
  // normalize axisX
  length =
      sqrtf(axisX[0] * axisX[0] + axisX[1] * axisX[1] + axisX[2] * axisX[2]);
  ...
  // normalize axisY
  length =
      sqrtf(axisY[0] * axisY[0] + axisY[1] * axisY[1] + axisY[2] * axisY[2]);
...
ESMatrix esMatrixScale(ESMatrix *matrix, float scale) {
  ...
  for (allElements = 0; allElements < 16; allElements++) {
    result.m[allElements / 4][allElements % 4] =
        matrix->m[allElements / 4][allElements % 4] * scale;
  }
